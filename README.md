...
- **工业级神经重排管线**：采用 Recall -> RRF Fusion -> Neural Rerank (BGE) 架构，实现语义匹配与字面匹配的完美均衡。
- **权威性加权算法**：独创“重排得分 * 法律位阶”评分体系，确保宪法、法律等上位法始终占据检索首位。
- **本地化模型部署**：全量支持离线/内网环境运行，嵌入与重排模型均采用本地驱动。
- **预览锚点精准同步**：点击法条标题，预览弹窗自动平滑滚动并锁定至原文特定条目，解决长文档查阅痛点。
- **法条参考多维筛选**：侧边栏支持实时关键词过滤，在大规模会话沉淀中亦能快速搜寻。
