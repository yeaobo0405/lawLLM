...
- **智能动态过滤 (Smart Dynamic Filtering)**：系统在回答生成结束后，会自动扫描全文引用的法律条目，侧边栏会自动剔除仅在推理过程中提及但未在最终建议中采纳的法条，确保参考信息的高度专业化。
- 法条引用自动添加"查看原文"按钮
- 点击按钮可预览法条完整内容
- 检索质量不佳时自动使用基座大模型回答
...
- **权威性检索加权**：根据法律位阶（宪法 > 法律 > 行政法规）智能调整优先级，确保最权威依据优先呈现。
- **预览锚点自动对齐**：点击侧边栏法条标题，文档预览弹窗可自动滚动至引用的“第 X 条”原文处，极大提升查阅效率。
- **法条参考多维筛选**：法条面板配备 Search Box，支持在多轮对话沉积的依据库中进行快速关键词过滤。
- **独立思维链展示**：智能展示 AI 思考过程（Reasoning），支持折叠与流式反馈，确保决策透明且不干扰阅读。
- **深层会话持久化**：支持历史会话中的思考过程、法条引用、文书草案的跨设备同步恢复。
- **智能文书起草**：基于案情描述，一键生成结构严谨、引用合法的民事、刑事及各类法律文书。
- **法律法规筛选**: 按法律名称、法条编号筛选
