...
1. **滚动分层摘要**: 实现 Layer 0/1/2 三级缓存策略，兼顾响应速度与长期记忆
2. **查询改写与指代消解**: 解决对话中的代词跳变问题，使 RAG 检索更精准
3. **混合检索**: 向量检索 + BM25 关键词检索，补齐语义与词法短板
4. **模型预加载**: 启动时预加载 Embedding 和 Reranker 模型，大幅降低首字延迟
5. **检索质量检测**: 自动评估检索结果相关性，动态切换 RAG 与全量大模型模式
6. **持久化对话记忆**: 使用 SQLite 存储对话历史，支持多设备/多会话同步
7. **法条精准匹配**: 支持各种复杂格式的法条引用识别与原文回溯
8. **分类过滤系统**: 法律法规与案例文书双轨管理，提供定制化的筛选交互
9. **用户认证**: 基于 JWT 的安全认证体系，实现严格的会话与权限隔离
